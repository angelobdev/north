...
with open("services/assets/public/gamedata/FurnitureData.json", encoding='utf-8') as f:
    data = json.load(f)
...
with open("services/assets/public/gamedata/ProductData.json", encoding='utf-8') as f:
    data = json.load(f)
...
with open("./tools/scripts/catalog_items.sql", "w", encoding='utf-8') as f:
    for furni in known_names:
        ...
        if furni["name"] is None:
            continue
        furni_name = sqlescape(repr(furni["name"]).encode(
            "latin-1", "ignore").decode("latin-1"))[:55]
        classname = sqlescape(repr(furni["classname"]).encode(
